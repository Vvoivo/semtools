...
use crate::common::validation::{validate_range_generic, validate_shard_different_peers};
use crate::segment::data_types::index::validate_integer_index_params;
...
impl Validate for crate::api::grpc::qdrant::AbortShardTransfer {
    fn validate(&self) -> Result<(), ValidationErrors> {
        validate_shard_different_peers(
            self.from_peer_id,
            self.to_peer_id,
...
impl Validate for grpc::Vector {
    fn validate(&self) -> Result<(), ValidationErrors> {
        ...
        match (indices, vectors_count) {
            ...
            (Some(indices), None) => {
                crate::sparse::common::sparse_vector::validate_sparse_vector_impl(&indices.data, data)
            }
            (None, Some(vectors_count)) => {
                crate::common::validation::validate_multi_vector_len(*vectors_count, data)
            }
            (None, None) => Ok(()),
        }
    }
...
impl Validate for grpc::SparseVector {
    fn validate(&self) -> Result<(), ValidationErrors> {
        ...
        crate::sparse::common::sparse_vector::validate_sparse_vector_impl(indices, values)
    }
}
...
impl Validate for grpc::MultiDenseVector {
    fn validate(&self) -> Result<(), ValidationErrors> {
        ...
        crate::common::validation::validate_multi_vector_by_length(&multivec_length)
    }
}
