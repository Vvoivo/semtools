...
use crate::common::either_variant::EitherVariant;
...
use crate::segment::index::query_estimator::estimate_filter;
use crate::segment::index::query_optimization::payload_provider::PayloadProvider;
use crate::segment::index::struct_filter_context::StructFilterContext;
use crate::segment::index::visited_pool::VisitedPool;
use crate::segment::index::{BuildIndexResult, PayloadIndex};
use crate::segment::json_path::JsonPath;
use crate::segment::payload_storage::payload_storage_enum::PayloadStorageEnum;
use crate::segment::payload_storage::{FilterContext, PayloadStorage};
...
impl StructPayloadIndex {
    ...
    fn load_from_db(
        ...
        #[cfg(feature = "rocksdb")]
        if crate::common::flags::feature_flags().migrate_rocksdb_payload_indices
    ...
    pub fn open(
        ...
        let config = if config_path.exists() {
            ...
            #[cfg(feature = "rocksdb")]
            {
                ...
                let skip_rocksdb = if is_appendable {
                    crate::common::flags::feature_flags().payload_index_skip_mutable_rocksdb
                } else {
                    crate::common::flags::feature_flags().payload_index_skip_rocksdb
                };
                if skip_rocksdb {
        ...
        let storage_type = if is_appendable {
            #[cfg(feature = "rocksdb")]
            {
                ...
                if !skip_rocksdb {
                    let rocksdb = crate::segment::common::rocksdb_wrapper::open_db_with_existing_cf(path)
                        .map_err(|err| {
        ...
        if db.is_none() && config.indices.any_is_rocksdb() {
            ...
            let rocksdb =
                crate::segment::common::rocksdb_wrapper::open_db_with_existing_cf(path).map_err(|err| {
                    OperationError::service_error(format!("RocksDB open error: {err}"))
                })?;
            db.replace(rocksdb);
        }
            ...
            && let Some(db) = index.db.take()
        {
            match Arc::try_unwrap(db) {
                Ok(db) => {
                    ...
                    let options = crate::segment::common::rocksdb_wrapper::make_db_options();
    ...
    pub fn restore_database_snapshot(
        ...
        crate::segment::rocksdb_backup::restore(snapshot_path, &segment_path.join("payload_index"))
    }
...
mod tests {
    ...
    use crate::segment::entry::{NonAppendableSegmentEntry, SegmentEntry};
    use crate::segment::index::payload_config::{IndexMutability, PayloadIndexType};
    ...
    use crate::segment::types::{Distance, PayloadSchemaType};
