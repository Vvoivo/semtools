...
use rand::RngExt;
use crate::segment::data_types::vectors::{DEFAULT_VECTOR_NAME, VectorInternal};
...
use crate::segment::types::{Distance, PayloadContainer};
...
fn test_apply_and_move_old_versions(
    ...
    segment1
        .upsert_point(
            100,
            123.into(),
            crate::segment::data_types::vectors::only_default_vector(&[0.0, 1.0, 2.0, 3.0]),
            &hw_counter,
        )
        .unwrap();
    segment1
    ...
    if segment_1_high_version {
        segment1
            .upsert_point(
                99999,
                99999.into(),
                crate::segment::data_types::vectors::only_default_vector(&[0.0, 0.0, 0.0, 0.0]),
                &hw_counter,
            )
            .unwrap();
    }
...
fn test_points_deduplication_bug() {
    ...
    segment1
        .upsert_point(
            2,
            10.into(),
            crate::segment::data_types::vectors::only_default_vector(&[0.0; 4]),
            &hw_counter,
        )
        .unwrap();
    segment2
...
fn test_points_deduplication_randomized() {
    ...
    let dir = Builder::new().prefix("segment_dir").tempdir().unwrap();
    let vector = crate::segment::data_types::vectors::only_default_vector(&[0.0; 4]);
