...
pub fn move_data(from_shard_path: &Path, to_shard_path: &Path) -> std::io::Result<()> {
    ...
    if from_wal_path.exists() {
        crate::common::fs::move_dir(&from_wal_path, &to_wal_path)?;
    }

    if from_segments_path.exists() {
        crate::common::fs::move_dir(&from_segments_path, &to_segments_path)?;
    }

    if from_newest_clocks_path.exists() {
        crate::common::fs::move_file(&from_newest_clocks_path, &to_newest_clocks_path)?;
    }

    if from_oldest_clocks_path.exists() {
        crate::common::fs::move_file(&from_oldest_clocks_path, &to_oldest_clocks_path)?;
    }

    if from_applied_seq_path.exists() {
        crate::common::fs::move_file(&from_applied_seq_path, &to_applied_seq_path)?;
    }
