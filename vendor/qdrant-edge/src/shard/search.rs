use crate::api::rest::SearchRequestInternal;
use crate::common::types::ScoreType;
use itertools::Itertools as _;
use crate::segment::data_types::vectors::{NamedQuery, NamedVectorStruct, VectorInternal};
use crate::segment::types::{Filter, SearchParams, WithPayloadInterface, WithVector};
use crate::segment::vector_storage::query::{ContextPair, ContextQuery, DiscoveryQuery, RecoQuery};
...
impl TryFrom<crate::api::grpc::qdrant::CoreSearchPoints> for CoreSearchRequest {
    type Error = tonic::Status;

    fn try_from(value: crate::api::grpc::qdrant::CoreSearchPoints) -> Result<Self, Self::Error> {
        let query = value
            .query
            .and_then(|query| query.query)
            .map(|query| {
                Ok(match query {
                    crate::api::grpc::qdrant::query_enum::Query::NearestNeighbors(vector) => {
                        let vector_internal = VectorInternal::try_from(vector)?;
                        QueryEnum::Nearest(NamedQuery::from(
                            crate::api::grpc::conversions::into_named_vector_struct(
                                value.vector_name,
                                vector_internal,
                            )?,
                        ))
                    }
                    crate::api::grpc::qdrant::query_enum::Query::RecommendBestScore(query) => {
                        QueryEnum::RecommendBestScore(NamedQuery {
                            query: RecoQuery::try_from(query)?,
                            using: value.vector_name,
                        })
                    }
                    crate::api::grpc::qdrant::query_enum::Query::RecommendSumScores(query) => {
                        QueryEnum::RecommendSumScores(NamedQuery {
                            query: RecoQuery::try_from(query)?,
                            using: value.vector_name,
                        })
                    }
                    crate::api::grpc::qdrant::query_enum::Query::Discover(query) => {
                    ...
                    crate::api::grpc::qdrant::query_enum::Query::Context(query) => {
                        let pairs = query
                            .context
                            .into_iter()
                            .map(try_context_pair_from_grpc)
...
fn try_context_pair_from_grpc(
    pair: crate::api::grpc::qdrant::ContextPair,
) -> Result<ContextPair<VectorInternal>, tonic::Status> {
    let crate::api::grpc::qdrant::ContextPair { positive, negative } = pair;
...
impl TryFrom<crate::api::grpc::qdrant::SearchPoints> for CoreSearchRequest {
    type Error = tonic::Status;

    fn try_from(value: crate::api::grpc::qdrant::SearchPoints) -> Result<Self, Self::Error> {
        let crate::api::grpc::qdrant::SearchPoints {
            collection_name: _,
            vector,
            filter,
            limit,
        ...
        if let Some(sparse_indices) = &sparse_indices {
            let crate::api::grpc::qdrant::SparseIndices { data } = sparse_indices;
        ...
        let vector_struct =
            crate::api::grpc::conversions::into_named_vector_struct(vector_name, vector_internal)?;
