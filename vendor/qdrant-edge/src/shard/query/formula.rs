...
use crate::api::grpc::DecayParamsExpression;
use crate::api::rest::GeoDistance;
use crate::api::{grpc, rest};
...
use crate::segment::types::{Condition, GeoPoint};
use serde::Serialize;
...
impl TryFrom<grpc::Formula> for FormulaInternal {
    ...
    fn try_from(formula: grpc::Formula) -> Result<Self, Self::Error> {
        ...
        let defaults = defaults
            .into_iter()
            .map(|(key, value)| {
                let value = crate::api::conversions::json::proto_to_json(value)?;
